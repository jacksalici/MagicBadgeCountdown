...
Options: `-t/--target "YYYY-MM-DD HH:MM"` (`24:00` = end of day), `-z/--tz` (`AoE`, `UTC`, `UTC+2`, `Europe/Rome`, `local`), `-m/--mode` (display mode, default 4; modes 9/10 are undocumented extensions some firmware ignores, showing a blank display), `-B/--brightness` (25/50/75/100), `-U/--upside-down` (flip the text 180° for badges worn clipped upside down).
